...
import com.shk.baseframe.demo.utils.cache.imgcode.ImgCodeCache;
import com.shk.baseframe.demo.utils.cache.imgcode.ImgCodeInfo;
...
public class CodeController {
    ...
    public ImgCodeInfo getImgCode(String codeKey, HttpServletRequest req) throws IOException {
        if(StringUtils.isNotBlank(codeKey)){
             ImgCodeCache.remove(codeKey);//将原有的二维码从缓存中移除掉
        }
        ...
        String imgUrl = "http://" + req.getServerName() + ":" + req.getServerPort() +  req.getContextPath() + "/code/getImg.do?codeKey=" + newCodeKey;
        imgCodeInfo.setCodeKey(newCodeKey);
        imgCodeInfo.setImgUrl(imgUrl);
        ImgCodeCache.addImgCode(newCodeKey, codeContents, imgUrl);
        return imgCodeInfo;
    }

    @ResponseBody
    @RequestMapping("/checkImgCode")
    public boolean checkImgCode(String codeKey, String codeContent) throws IOException {
        return ImgCodeCache.checkImgCode(codeKey, codeContent);
    }


    @RequestMapping("/getImg")
    public void getImg(HttpServletRequest req, HttpServletResponse resp, String codeKey) throws IOException {
        ImgCodeInfo imgCodeInfo = ImgCodeCache.getImgCodeInfo(codeKey);
