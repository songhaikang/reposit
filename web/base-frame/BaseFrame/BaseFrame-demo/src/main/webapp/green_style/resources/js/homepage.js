...
function _del(a)
{
	msg="您想不在首页上显示该模块吗?\n\n关闭后可通过功能菜单中 “个人事务 -> 个人设置 -> 首页设置” 来重新恢复显示";
...
function initHead(a)
{
	...
	{//对所有c_x的子结点遍历，其实也就是命名为m_x的div标签。最后一个div标签有其它用处，
		...
		for(var j=0;j<colArray[i].childNodes.length-1;j++)
		{
			...
			if(more_i)      // _op ID鼠标感应效果，放上去显示，离开隐藏。
			{
...
function aI()
{
	...
	_sendXMLRequest("/function/personsetting/homepage/dragconfig.php?MYTABLE="+escape(a),null)
}
